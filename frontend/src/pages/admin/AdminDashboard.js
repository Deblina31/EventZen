...
const AdminDashboard = () => {
  ...
  const fetchAllData = useCallback(async () => {
    ...
    const safeFetch = async (url, setter, label) => {
      try {
        ...
        console.error(
          `${label} Fetch Error (${err.response?.status}):`,
    ...
    await Promise.all([
      safeFetch(
        "http://localhost:8080/admin/users?size=100",
        setUsers,
        "Users",
      ),
  ...
  const handleDeleteVenue = async (id) => {
    ...
    const venueId = id;
    try {
      await axios.delete(`http://localhost:5193/venues/${venueId}`, {
        headers: getHeaders(),
      });
      setVenues((prev) => prev.filter((v) => (v.id || v.Id) !== venueId));
    } catch (err) {
      alert("Venue delete failed");
    }
  };
              ...
              <tbody>
                {filteredUsers.map((u) => (
                  <tr key={u.id}>
                    ...
                    <td>
                      <span
                        className={`badge ${u.role === "VENDOR" ? "vendor-bg" : "user-bg"}`}
                      >
                        {u.role}
                      </span>
                    </td>
                ...
                {events.map((e) => (
                  <tr key={e.id}>
                    ...
                    <td>
                      {(() => {
                        const rawDate =
                          e.eventDate || e.date || e.dateTime || e.startTime;
              ...
              <tbody>
                {venues.map((v) => (
                  <tr key={v.id || v.Id}>
                    ...
                    <td>
                      <button
                        onClick={() => handleDeleteVenue(v.id || v.Id)}
                        className="delete-btn-table"
                      >
                        Delete
                      </button>
